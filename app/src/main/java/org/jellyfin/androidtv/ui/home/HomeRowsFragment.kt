...
class HomeRowsFragment : RowsSupportFragment(), AudioEventListener, View.OnKeyListener {
    ...
    // Special rows
    private val notificationsRow by lazy {
        NotificationsHomeFragmentRow(
            lifecycleScope,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                ...
                // Add rows in order
                notificationsRow.addToRowsAdapter(
                    requireContext(),
                    cardPresenter,
                    adapter as MutableObjectAdapter<Row>
                )
                nowPlaying.addToRowsAdapter(
                    requireContext(),
                    cardPresenter,
                    adapter as MutableObjectAdapter<Row>
                )
                for (row in rows) row.addToRowsAdapter(
                    requireContext(),
                    cardPresenter,
                    adapter as MutableObjectAdapter<Row>
                )
